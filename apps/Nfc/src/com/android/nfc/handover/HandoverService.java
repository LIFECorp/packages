                ...
                if (action.equals(ACTION_BT_OPP_TRANSFER_DONE)) {
                    ...
                    if (handoverStatus == HANDOVER_TRANSFER_STATUS_SUCCESS) {
                        ...
                        if (uri.getScheme() == null) {
                            /// M: @ {
                            //ALPS01413917, Android bug, send @#%.jpg issue
                            Log.d(TAG, "BT input uriString:"+uriString); 
                            Log.d(TAG, "error   uri.getPath():"+uri.getPath()); 
                            uri = Uri.fromFile(new File(uriString));
                            Log.d(TAG, "correct uri.getPath():"+uri.getPath()); 
                            /// }                                                    
                        }
                        ...
                        if (!incoming) {                            
                            try {
                                String filePath = Util.getFilePathByString(TAG,context,uriString);
                                File f = new File(filePath);
                                FilePushRecord.sendBtOutgoingIntent(context,filePath, mimeType, true, f.length(), f.length());
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
