...
public class MediaPlaybackService extends Service {
    ...
    private void notifyChange(String what) {
        ...
        /**
         * M: Changed to avoid ANR caused by database and bitmap generating operation.@{
         */
    ...
    /**
     * M: Added to avoid ANR caused by database and bitmap generating operation.@{
     */
    private final class NotifyTask extends AsyncTask<Void,Void,Boolean>{
        private long mAudioId = -1;
        private String mAudioArtistName = null;
        private String mAudioAlbumName = null;
        private String mAudioTrackName = null;
        private boolean mAudioPlayState = false;
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            mAudioId = Long.valueOf(currentId);
            mAudioArtistName = getArtistName();
            mAudioAlbumName = getAlbumName();
            mAudioTrackName = getTrackName();
            mAudioPlayState = isPlaying();
            MusicLogUtils.d(TAG, "doInBackground(" + (mAction != null ? mAction : "null") + ")");
            if (mAction != null) {
                if (mAction.equals(META_CHANGED)) {
                    mBgBitmap = MusicUtils.getArtwork(
                            MediaPlaybackService.this.getApplicationContext(), getAudioId(),
                            getAlbumId(), true);
                }
                return Boolean.TRUE;
            } else {
        ...
        protected void onPostExecute(Boolean result) {
            if(result == null || result == Boolean.FALSE){
                MusicLogUtils.d(TAG, "onPostExecute() result is null || result is false");
                return;
            }else{
                long currentId = getAudioId();
                Intent i = new Intent(mAction);
                i.putExtra("id", mAudioId);
                i.putExtra("artist", mAudioArtistName);
                i.putExtra("album",mAudioAlbumName);
                i.putExtra("track", mAudioTrackName);
                i.putExtra("playing", mAudioPlayState);
                
                if (QUIT_PLAYBACK.equals(mAction)) {
                    mPlayPos = -1;
                    mPlayListLen = 0;
                    ...
                    if (mAction.equals(PLAYSTATE_CHANGED)) {
                    ...
                    } else if (mAction.equals(META_CHANGED)) {
                        if (currentId != mPreviousId) {
                            MusicLogUtils.d("test", "getTrackName:" + getTrackName());
                            ...
                            if (mBgBitmap != null) {
                                ed.putBitmap(MetadataEditor.BITMAP_KEY_ARTWORK, mBgBitmap);
                            }
                            ed.apply();
                ...
                if (mAction.equals(QUEUE_CHANGED)) {
                    saveQueue(true);
                } else {
                    saveQueue(false);
                }
                // Share this notification directly with our widgets
                mAppWidgetProvider.notifyChange(MediaPlaybackService.this, mAction);
                // / M: AVRCP14
                notifyBTAvrcp(mAction);
            }
        }
    }
    /**
