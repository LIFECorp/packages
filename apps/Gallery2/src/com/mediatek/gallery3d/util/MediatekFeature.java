...
//        prepareImageOptions(context);
    ...
    public static String getWhereClause(int mtkInclusion) {
        ...
        String whereClauseMav = null;
        ///M:Exclude Mpo file,if the device do not support Mpo format. 
        if (!supportMpo) {
            whereClauseMav = MpoHelper.getMavWhereClause(mtkInclusion);
        }
        /// @}
        String whereGroup = null;
        if (null != whereClauseMav) {
            whereGroup = (null == whereGroup) ? whereClauseMav :
                         "(" + whereGroup + ") AND (" + whereClauseMav +")";
        }
    ...
    private static final boolean supportThumbnailMAV = FeatureOption.MTK_MAV_PLAYBACK_SUPPORT && IS_HEAVY_FEATURE_SUPPORTED;
    ...
    // M: added for rendering sub-type overlay on micro-thumbnail, e.g. MAV/...
    ...
    public static String get2kVideoWhereClause() {
        ...
        if (support2kVideo && needHide2kVideo) {
            return " (" + VideoColumns.WIDTH + " <= 1920" + " AND " + VideoColumns.HEIGHT + " <= 1920) " +
    ...
    public static String getMediatekWhereClause(String whereClause) {
        String whereClause2K = get2kVideoWhereClause();
        if (whereClause2K == null) {
            return whereClause;
        } else {
            String clause = (null == whereClause) ? get2kVideoWhereClause() :
                "(" + whereClause + ") AND (" + get2kVideoWhereClause() +")";
            return clause;
        }
    }
    // @}
