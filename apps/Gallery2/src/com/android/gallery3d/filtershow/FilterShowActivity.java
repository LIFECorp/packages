...
public class FilterShowActivity extends FragmentActivity implements OnItemClickListener,
    ...
    public void showDefaultImageView() {
        mEditorPlaceHolder.hide();
        ///M:add to resolve null exception
        if(mImageShow != null) {
    ...
    public void saveImage() {
        ///M: generally isFileExistDetecting=false, except that the performance is very low {
        if (mIsFileExistDetecting) {
            return;
        }
        /// @}
        if (mImageShow.hasModifications()) {
            ...
            if ((intent != null) && (intent.getBooleanExtra(NEED_SAVE_AS, false))) {
                /// M: show saving progress for continus shot pictures. @{
    ...
    private boolean mIsFileExistDetecting = false;
    public boolean isFileExistDetecting() {
        return mIsFileExistDetecting;
    }
    private void finishIfInvalidUri() {
        if (!needCheckInvalid) return;
        
        mIsFileExistDetecting = true;
                    ...
                    try {
                        ...
                        if ((cur == null) || (cur.getCount() == 0)) {
                            runOnUiThread(new Runnable() {
                                public void run() {
                                    ErrMsgConfirmer.comfirmMessage(
                                            FilterShowActivity.this,
    ...
    /// M: add for ANR isue. Click back key ,while apply filter in sub thread.
