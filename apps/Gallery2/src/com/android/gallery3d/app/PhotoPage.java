...
public class PhotoPage extends ActivityState implements
    ...
    public void onCreate(Bundle data, Bundle restoreState) {
        ...
        ///M:Exclude Mpo file,if the device do not support Mpo format. 
        if (!mIsMavSupported) {
            mMtkInclusion |= MpoHelper.getInclusionFromData(data);
        }
        ///@}
        ...
        Path itemPath = null; 
        if (mIsDrmSupported ||!mIsMavSupported ) {
        ...
        // so place it there
        if(mIsMavSupported) {
            MtkLog.d(TAG, "Check FeatureOption.MTK_MAV_PLAYBACK_SUPPORT="+FeatureOption.MTK_MAV_PLAYBACK_SUPPORT);
            mMavSeekBar = (MavSeekBar) ((Activity) mActivity).findViewById(R.id.seekbar_mav);
    ...
    public void setCameraPath(String setPath) {
        ...
        if(mScreenNailSet == null || mScreenNailItem == null) {
            return;
        }
...
//            }, mActivity);
    ...
    private void launchPhotoEditor() {
        ...
        ///M: create new task when launch photo editor from camera,
    ...
    protected boolean onCreateActionBar(Menu menu) {
        ...
        mActionBar.setShareActionProviderListener(this);
        updateMenuOperations();
        updateActionBarTitle();
        return true;
    }
    ...
    protected boolean onItemSelected(MenuItem item) {
        ...
        switch (action) {
            ...
            case R.id.action_picture_quality: {
                ...
                Intent intent = new Intent(PictureQualityActivity.ACTION_Main_QuickContactBadge);
    ...
    // M: When user is sliding, not show seek bar @{
    private boolean mSeekBarVisible = true;
    public void onSeekBarVisible(boolean visible) {
        if (mSeekBarVisible == visible) return;
        mSeekBarVisible = visible;
        updateMavSeekBar();
        updatePanoramaSeekBar();
    }
    // @}
    ...
    private void updatePanoramaSeekBar() {
        ...
        if (mCurrentPhoto.isPanorama()
                ...
                && !mPhotoView.getFilmMode()
                // No bars if we are showing camera preview
                && (mAppBridge == null || mCurrentIndex != 0)
                && mSeekBarVisible) {
    ...
    private void setMTKCurrentPhotoByIntent(Intent intent) {
        ...
        if (photoEditPath != null) {
            ...
            if (string != null) {
                /// M: mark the cache for edited image out of date
    ...
    public boolean onShareTargetSelected(ShareActionProvider source, Intent intent) {
        ...
        makeShareProviderIgnorAction(intent);
        return true;
    }

    private void makeShareProviderIgnorAction(Intent intent) {
