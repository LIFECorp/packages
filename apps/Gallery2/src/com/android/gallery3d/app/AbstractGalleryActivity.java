...
public class AbstractGalleryActivity extends Activity implements GalleryContext {
    ...
    private JobLimiter mVideoMicroThumbDecoder;
    public JobLimiter getVideoMicroThumbDecodingPool(ThreadPool attachedThreadPool) {
        // not strict singleton compared to getThreadPool(), but it's generally sufficient
        // or we would implement this like getThreadPool() and modify more files
        if (mVideoMicroThumbDecoder == null) {
            final int VIDEO_MICRO_THUMB_DECODER_JOB_LIMIT = FeatureOption.MTK_LCA_RAM_OPTIMIZE ? 1
                    : 2;
            Log.v(TAG, "create VideoMicroThumbDecodingPool with concurrency "
                    + VIDEO_MICRO_THUMB_DECODER_JOB_LIMIT);
            Log.v(TAG, "created VideoMicroThumbDecodingPool with thread pool "
                    + attachedThreadPool);
            mVideoMicroThumbDecoder = new JobLimiter(attachedThreadPool,
                    VIDEO_MICRO_THUMB_DECODER_JOB_LIMIT);
        }
        return mVideoMicroThumbDecoder;
    }
    /// @}
}
