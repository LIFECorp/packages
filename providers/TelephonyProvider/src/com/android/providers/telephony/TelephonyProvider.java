...
//MTK-END [mtk04170][111215]
        ...
        private void createMultiSimDb(SQLiteDatabase db) {
            String columns = null;
            if (FeatureOption.MTK_OMACP_SUPPORT == true) {
                columns = "(_id INTEGER PRIMARY KEY,"
                    + "name TEXT,"
            ...
            for (int i=1; i<PhoneConstants.GEMINI_SIM_NUM; ++i) {
                //start with table2
                db.execSQL("CREATE TABLE " + CARRIERS_TABLE_GEMINI[i] + columns);
            }
        }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (oldVersion < (13 << 16 | 6)) {
                //version 12 is support dual sim
                //version 13 is support multiple sim
                if (FeatureOption.MTK_GEMINI_SUPPORT) {
                    Log.d(TAG, "Rename multiple sim apn database");
                    try {
                        db.execSQL("ALTER TABLE carriers_gemini RENAME TO " + CARRIERS_TABLE_GEMINI[1] + ";");
                    } catch  (SQLException e) {
                        e.printStackTrace();
                        Log.e(TAG, "Rename carriers_gemini table fail.");
                    }
                }
